...
func (iw *IngestionWorker) HiveSearch(ctx context.Context, args HiveSearchArguments) (out HiveSearchResponse, resultErr error) {
	...
	for _, point := range unique {
		...
		response.Results = append(response.Results, HiveSearchResult{
			...
			SourceFormat:  payloadString(point.Payload, "source_format", ""),
			SourceLocator: payloadString(point.Payload, "source_locator", ""),
			CanonicalHash: payloadString(point.Payload, "canonical_hash", ""),
