...
> Releases novas usam o fluxo assinado de [entrega e deploy](delivery-and-deployment.md): o servidor promove bundles imutáveis com backup/health check/rollback e os clientes baixam o binário fixado pelo `hive_instance`. Os comandos manuais abaixo continuam explicando o bootstrap e a arquitetura, mas não devem ser usados para copiar binários entre repositórios.
...
# Em produção, instale o bootstrap conforme delivery-and-deployment.md e promova:
sudo /usr/local/sbin/hive-pull-release vX.Y.Z --initial
docker compose -p hive-server \
...
Na máquina do primeiro writer, use preferencialmente o `hive_instance`, fixando uma release publicada e assinada. O instalador baixa o asset adequado e verifica o checksum. A compilação abaixo fica reservada ao desenvolvimento da revisão candidata; não copie seu resultado manualmente para o repositório cliente. São necessários Go 1.25+ e compilador C/C++ para CGO/tree-sitter:
