...
| Baixa a versão definida em `hive-release.json`. | Publique e fixe uma release assinada antes de entregar a instância. |
...
Antes disso, confira se `hive-release.json` aponta para uma release existente. O instalador detecta SO/arquitetura, baixa o asset correspondente e verifica seu SHA-256; não entregue nem versione executáveis manualmente. Em repositório privado, prepare `config/github-release.token` com acesso somente de leitura e modo `0600`.
