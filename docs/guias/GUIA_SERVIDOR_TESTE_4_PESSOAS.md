...
Para novas releases, prefira o fluxo assinado descrito em [entrega e deploy](../operations/delivery-and-deployment.md). Este guia conserva os passos explícitos do laboratório, mas binários não devem mais ser commitados nem copiados manualmente para `hive_instance`.
...
O programa sintético permanece com escopo `unknown`. Isso permite o teste de memória, sem conceder autorização sobre ativos. Para programas reais, uma pessoa responsável por programa deve preparar, revisar e aprovar `scope.json` pelo [procedimento de escopo](../spec/contracts/scope-manifest.md).
...
Depois da calibração, siga o [ensaio de quatro pessoas](../operations/four-person-trial.md): dois grupos, dois programas autorizados e troca de contexto. Exporte `audit report` de cada computador; a auditoria é local, não centralizada automaticamente no servidor.
...
No servidor, para acompanhar ou reiniciar uma instalação manual deste laboratório:
...
Este roteiro prepara o laboratório e os testes iniciais. O aceite operacional completo continua dependente da execução real, incluindo compartilhamento entre máquinas, revogação e restauração. O [guia canônico de servidor](../operations/server-multiuser-guide.md) detalha a implantação geral.
